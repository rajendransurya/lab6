...
def doAdd(stub, debug=False):
    response = stub.PerformAdd(grpc_rest_pb2.addMsg(a=5, b=10))
    if debug:
...
def doRawImage(stub, debug=False):
    ...
    response = stub.ImageDimensions(grpc_rest_pb2.rawImageMsg(img=img))
    if debug:
...
def doDotProduct(stub, debug=False):
    ...
    response = stub.PerformDotProduct(grpc_rest_pb2.dotProductMsg(a=a, b=b))
    if debug:
...
def doJsonImage(stub, debug=False):
    ...
    response = stub.JsonImageDimensions(grpc_rest_pb2.jsonImageMsg(img=encoded_string))
...
if __name__ == '__main__':
    ...
    with grpc.insecure_channel(str(addr)) as channel:
        stub = grpc_rest_pb2_grpc.Grpc_lab6Stub(channel)

        if cmd == 'rawImage':
            start = time.perf_counter()
            for x in range(reps):
                doRawImage(stub,debug=False)
                # doRawImage(stub,debug=True)
